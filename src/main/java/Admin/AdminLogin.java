...
public class AdminLogin extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	/// Retrieve the secret key parameter from the URL
        String secretParam = request.getParameter("secretKey");

        // Check if the secret parameter is not present or incorrect
        if (secretParam == null || !secretParam.equals("yourSecretKey")) {
            // Unauthorized access, redirect to an error page or display a message
            response.sendRedirect("error.jsp"); // You can create an "error.jsp" page
        } else {
            // Authorized staff members can access the staffLogin.jsp
